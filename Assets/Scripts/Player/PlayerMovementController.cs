...
public sealed class PlayerMovementController : MonoBehaviour
{
    ...
    [SerializeField, Min(0f)] private float groundAcceleration = 25f;
    ...
    private void FixedUpdate()
    {
        ...
        bool canUseCoyoteTime = Time.time - lastGroundedTime <= CoyoteTimeDuration;
        if (hasBufferedJump && canUseCoyoteTime)
        {
            Vector3 jumpVelocityVector = body.linearVelocity;
            jumpVelocityVector.y = jumpVelocity;
            body.linearVelocity = jumpVelocityVector;
    ...
    private void LateUpdate()
    {
        if (cameraTarget != null)
        {
            cameraTarget.rotation = cameraTargetWorldRotation;
        }
    }
    ...
    private Vector3 GetCameraRelativeDirection(Vector2 input)
    {
        ...
        Transform reference = movementCamera != null ? movementCamera : transform;
    ...
    private bool CheckGrounded()
    {
        ...
        float castRadius = Mathf.Min(groundCheckRadius, bounds.extents.x, bounds.extents.z);
        castRadius = Mathf.Max(castRadius, 0.01f);
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if (!hitTransform.IsChildOf(transform) && groundHits[i].normal.y >= 0.5f)
            {
                return true;
            }
        }
